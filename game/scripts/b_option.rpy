...
label accident_option:
    ...
    call expression accident1
    
    # 触发第二个随机事件
    $ accident2 = selected_accident[1]
    call expression accident2
...
label events_option:
    ...
    # 根据信号量依次触发每个事件
    if call_low_energy:
        call b_event_low_energy
    if call_low_social:
        call b_event_low_social
    if call_high_social:
        call event_high_social
    if call_low_study:
        call b_event_low_study
    if call_low_mental:
        call b_event_low_mental
    if call_low_finance:
        call b_event_low_finance
    if call_low_health:
        call b_event_low_health
