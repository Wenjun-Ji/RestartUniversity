label a_stuactivity:
    scene bg activity_center  # 这个是大学校园背景
    python:
        scen_desc_1 = """
        时间：大一期间  
        地点：学生活动中心
        ...
        # 调用AI生成新的大学场景剧情
        new_plot = text_ai.invoke(scen_desc_1, plot_history, value_dict)
    ...
    $ res = select_ai.invoke("我来到学生活动中心", value_dict, plot_history, ["club_meeting", "hold_activities","conflict"])
    if res == "club_meeting":
...
label a_joindiscussion:
    scene bg classroom  # 这个是家中客厅的背景，你可以根据实际场景设置
    ...
    python:
        ...
        # 调用AI更新属性
        li = value_ai.invoke(new_plot, "E:-10~-5,A:+3~+5,B:0~+3,M:+5~+10,S:+10~+20,F:0~0", value_dict)
...
label a_justlisten:
    ...
    python:
        ...
        # 调用AI更新属性
        li = value_ai.invoke(new_plot, "E:-3~0,A:+3~+5,B:0~+3,M:+3~+5,S:0~+3,F:0~0", value_dict)
...
label a_beorganizer:
    scene bg auditorium  # 这个是礼堂/舞台的背景
    ...
    python:
        ...
        # S社交能力显著提升
        # F财务状况不变
        li = value_ai.invoke(new_plot, "E:-15~-10,A:-5~-3,B:-5~-3,M:-5~+10,S:+15~+20,F:0~0", value_dict)
...
label a_beaudience:
    scene bg auditorium  # 这个是礼堂/舞台的背景，从观众视角
    ...
    python:
        ...
        # S社交状况略有提升（集体活动）
        # F财务状况略降（购票）
        li = value_ai.invoke(new_plot, "E:-3~-1,A:0~0,B:0~0,M:+3~+5,S:+1~+3,F:-2~-1", value_dict)
...
label a_joinclub:
    scene bg classroom  # 这个是教室背景，表示与课业的冲突
    ...
    python:
        ...
        # S社交状况显著提升
        # F财务状况略降（活动支出）
        li = value_ai.invoke(new_plot, "E:-8~-5,A:-15~-10,B:+2~+4,M:-2~+5,S:+8~+12,F:-5~-3", value_dict)
...
label a_studyhard:
    scene bg library  # 这个是图书馆背景，表示专注学习
    ...
    python:
        ...
        # S社交状况下降（错过活动）
        # F财务不变
        li = value_ai.invoke(new_plot, "E:-12~-8,A:+10~+15,B:-3~-1,M:-3~+3,S:-8~-5,F:0~0", value_dict)
...
label a_relax:
    ...
    python:
        ...
        # S社交状况下降（独处娱乐）
        # F财务略降（娱乐消费）
        li = value_ai.invoke(new_plot, "E:+5~+10,A:-15~-10,B:-2~-1,M:-5~+5,S:-6~-3,F:-5~-3", value_dict)
