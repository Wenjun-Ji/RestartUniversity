...
label a_freshtest:
    ...
    scene bg examhall  # 这个是大学校园背景
    python:
        scen_desc_1 = """
        时间：大学开学一个月后
        地点：大学教室
        人物：我，室友，老师
        场景描述：今天是开学以来的第一次考试，我感到既紧张又兴奋。室友和老师都鼓励我，提醒我保持冷静，仔细审题。我深吸一口气，准备迎接这次挑战。
    ...
    # 玩家互动选项
    $ res = select_ai.invoke("", value_dict, plot_history, ["good_result", "bad_result"])
    if res == "good_result":
        jump a_goodresult
    if res == "bad_result":
        jump a_badresult
...
label a_goodresult:
    ...
    python:
        ...
        # 调用AI更新属性
        li = value_ai.invoke(new_plot, "E:+1~+5,A:+10~+20,B:+3~+5,M:+10~+20,S:+5~+10,F:0~0", value_dict)
...
label a_badresult:
    ...
    python:
        ...
        # 调用AI更新属性
        li = value_ai.invoke(new_plot, "E:-5~-3,A:-10~-5,B:-3~-1,M:-10~-5,S:-5~-3,F:0~0", value_dict)
