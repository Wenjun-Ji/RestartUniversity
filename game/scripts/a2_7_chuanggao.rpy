label a_chuanggao:
    ...
    $ res = select_ai.invoke("我思考是否去操场跑步", value_dict, plot_history, ["normal_run", "rainy_day","lazy_run"])
    if res == "normal_run":
...
label a_normal_run:
    ...
    python:
        ...
        # 调用AI更新属性
        li = value_ai.invoke(new_plot, "E:-5~-3,A:0~0,B:+5~+10,M:+5~+10,S:+5~+10,F:0~0", value_dict)
...
# 下雨天场景
label a_rainy_day:
    scene bg rainy_playground  # 这个是大学校园背景
    python:
        scen_desc_1 = """
        时间：大一期间  
        地点：下雨的操场  
        ...
        # 调用AI更新属性
        li = value_ai.invoke(new_plot, "E:+3~+5,A:0~0,B:-3~0,M:-3~0,S:0~0,F:0~0", value_dict)
...
label a_lazy_run:
    ...
    python:
        ...
        # 调用AI更新属性
        li = value_ai.invoke(new_plot, "E:+3~+5,A:0~0,B:-3~0,M:-3~0,S:-3~0,F:0~0", value_dict)
