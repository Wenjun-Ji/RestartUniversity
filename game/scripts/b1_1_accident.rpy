    ...
    menu:
        "借钱吃饭":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:0~0,A:0~0,B:0~0,M:0~0,S:+5~+10,F:-5~-1", value_dict)
        ...
        "不吃饭，直接补办一卡通":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-10~-5,A:0~0,B:0~0,M:-5~0,S:-5~0,F:0~0", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            jump event_lost_card_replace_card

# 交通事故
label b_accident_traffic_accident:
    scene bg street  # 街道背景
    ...
    menu:
        "去医院检查":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-10~-5,A:0~0,B:-5~-1,M:-5~0,S:0~0,F:-5~0", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
        ...
        "不理会，继续前行":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:0~0,A:0~0,B:-20~-15,M:-10~-5,S:0~0,F:0~0", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            jump event_traffic_accident_ignore

# 生病
label b_accident_illness:
    scene bg dorm_room  # 宿舍背景
    ...
    menu:
        "去医务室看病":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-5~-1,A:0~0,B:-5~-1,M:0~0,S:0~0,F:-5~-1", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
        ...
        "自己扛着，不去医务室":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-10~-5,A:0~0,B:-20~-15,M:0~0,S:0~0,F:0~0", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            jump event_illness_self_heal

# 网络诈骗
label b_accident_scam:
    scene bg internet  # 网络背景
    ...
    menu:
        "报警处理":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-10~-5,A:0~0,B:0~0,M:+1~+5,S:0~0,F:0~0", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
        ...
        "接受损失":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-5~-1,A:0~0,B:0~0,M:-10~-5,S:-5~-1,F:-20~-15", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            jump event_scam_accept_loss

# 运动受伤
label b_accident_activity_injury:
    scene bg sports_field  # 操场背景
    ...
    menu:
        "继续比赛":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-10~-5,A:0~0,B:-20~-15,M:0~0,S:+5~+10,F:0~0", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
        ...
        "退出比赛，去医务室":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-5~-1,A:0~0,B:-10~-5,M:-5~0,S:0~0,F:-3~-1", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            jump event_activity_injury_quit

#遇到聊得来的朋友
label b_accident_meet_friend:
    scene bg hiking_path  # 户外背景
    ...
    menu:
        "加深友谊":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:0~0,A:0~0,B:0~0,M:+5~+10,S:+15~+20,F:0~0", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
        ...
        "保持礼貌，但不继续交流":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:0~0,A:0~0,B:0~0,M:0~0,S:-5~-1,F:0~0", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            jump event_meet_friend_polite

#论文被评为优秀作品
label b_accident_paper_selected:
    scene bg library  # 图书馆背景
    ...
    menu:
        "庆祝自己的成功":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-5~-1,A:+5~+10,B:0~0,M:+10~+15,S:+5~+10,F:-5~-1", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
        ...
        "继续低调前行":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:0~0,A:+10~+15,B:0~0,M:+5~+10,S:0~0,F:0~0", value_dict)
                #renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
