...
label a_gaokao_good:
    ...
    python:
        ...
        li = value_ai.invoke(new_plot, "E:+10~+15,A:+0~+0,B:+3~+5,M:+15~+20,S:+5~+8,F:+0~+0", value_dict)
...
label a_plan_summer:
    ...
    menu:
        "管他呢，先玩再说":
            python:
                # 更新剧情历史
                plot_history.extend(["旁白：主角选择了“放松，彻底享受暑假”这个选项"])# 这个地方可以人工强调选择的后果，也可让AI自己判断
                # 调用AI更新属性：放松提升精力（E），但学业进度下降（A），身体和心理健康提升（B、M），财务状况下降（F）
                li = value_ai.invoke(new_plot, "E:+12~+15,A:-5~-8,B:+3~+5,M:+10~+15,S:+3~+5,F:-10~-15", value_dict)
                # 显示属性变化
                renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            pause
            # 跳转到下一个场景
        ...
        "只要卷不死，就往死里接着卷":
            python:
                # 更新剧情历史
                plot_history.extend(["旁白：主角选择了“提前学习大学课程”这个选项"])
                # 调用AI更新属性：学习增加学业进度（A），但精力和心理健康下降（E、M）
                li = value_ai.invoke(new_plot, "E:-5~-8,A:+15~+20,B:+0~+0,M:-10~-15,S:+0~+0,F:+0~+0", value_dict)
                # 显示属性变化
                renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            pause 
            # 跳转到下一个场景
        ...
        "先挣点小钱儿":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-5~-8,A:-5~-7,B:+0~+0,M:+2~+5,S:+7~+10,F:+20~+30", value_dict)
                # 显示属性变化
                renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            pause 
            # 跳转到下一个场景
    ...
    menu:
        "学车":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-5~+0,A:+0~+0,B:+5~+7,M:+2~+5,S:+0~+0,F:-8~-12", value_dict)
                # 显示属性变化
                renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            pause
            # 跳转到下一个场景
        ...
        "学习大学的知识":
            python:
                ...
                li = value_ai.invoke(new_plot, "E:-5~-8,A:+12~+18,B:+0~+0,M:-5~-10,S:+0~+0,F:+0~+0", value_dict)
                # 显示属性变化
                renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            pause 
            # 跳转到下一个场景
        ...
        "挣点小钱":
            python:
                ...
                plot_history.extend(["旁白：主角选择了“暑假打工”这个选项"])
                # 调用AI更新属性：打工增加财务状况（F）和社交能力（S），但精力下降（E），学业进度下降（A）
                li = value_ai.invoke(new_plot, "E:-5~-8,A:-4~-6,B:+0~+0,M:+5~+8,S:+10~+15,F:+15~+25", value_dict)
                # 显示属性变化
                renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            pause 
            # 跳转到下一个场景
...
label a_walk_alone:
    ...
    menu:
        "在大学享受自由继续放松":
            python:
                # 更新剧情历史
                plot_history.extend(["旁白：主角选择了“在大学享受自由继续放松”这个选项"])# 这个地方可以人工强调选择的后果，也可让AI自己判断
                # 调用AI更新属性
                li = value_ai.invoke(new_plot, "E:+3~+5,A:-5~-8,B:+7~+10,M:+3~+5,S:+5~+7,F:+0~+0", value_dict)
                # 显示属性变化
                renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            pause
            # 跳转到下一个场景
        ...
        "度过一个精心规划的大学生涯":
            python:
                # 更新剧情历史
                plot_history.extend(["旁白：主角选择了“度过一个精心规划的大学生涯”这个选项"])
                # 调用AI更新属性
                li = value_ai.invoke(new_plot, "E:+5~+7,A:+10~+15,B:+3~+5,M:+5~+8,S:+0~+0,F:+0~+0", value_dict)
                # 显示属性变化
                renpy.say(None, "\n".join([f"{key}：{value}" for key, value in li.items()]))
            pause 
            # 跳转到下一个场景
